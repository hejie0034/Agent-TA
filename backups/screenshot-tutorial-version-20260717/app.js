...
function startSession(createRecord) {
  ...
  addMessage("assistant", welcome, "uLearning教师操作助手");
  if (createRecord) persistCurrentSession();
}

async function ask(question) {
  addMessage("user", question, "教师");
  const guided = findGuidedResponse(question);
  if (guided) {
    window.setTimeout(() => {
      addMessage("assistant", guided.answer, "uLearning教师操作助手", null, null, null, {
        prompt: guided.prompt,
        actions: guided.actions,
        screenshots: (guided.screenshotIds || []).map(findScreenshotGuideById).filter(Boolean)
      }, question);
  ...
  window.setTimeout(() => {
    ...
    const screenshot = findScreenshotGuide(question);
    const video = null;
    let answer = addContextualLead(result.answer, result, question);
    if (!result.matched && screenshot) {
      answer = `${screenshot.title}可以按下面截图步骤操作。`;
    }
    const next = getContextualGuidance(result, question, screenshot);
    addMessage(
      "assistant",
      answer,
      "uLearning教师操作助手",
...
function renderMessage(message) {
  renderMessageToContainer(message, nodes.messages);
  prepareScreenshotImages(nodes.messages.lastElementChild);
  nodes.messages.scrollTop = nodes.messages.scrollHeight;
}
...
function getClickableLinks(message) {
  ...
  faqItems.forEach((item) => {
    links.push([item.title.replace(/^如何/, "").replace(/[？?]$/, ""), item.title]);
    links.push([item.title, item.title]);
    (item.keywords || []).forEach((keyword) => links.push([keyword, item.title]));
  });
  videoGuides.forEach((video) => {
    links.push([video.title, video.question]);
    (video.aliases || [video.title]).forEach((alias) => links.push([alias, video.question]));
  });
  screenshotGuides.forEach((guide) => {
    links.push([guide.title, guide.question]);
    (guide.aliases || [guide.title]).forEach((alias) => links.push([alias, guide.question]));
  });
  const seen = new Set();
...
function getGuidanceTitle(prompt) {
  ...
  return cleanSentence(rawPrompt);
}
